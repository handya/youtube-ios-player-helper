...
NSString static *const kYTPlayerUrlRegexPattern = @"^http(s)://(www.)youtube.com/watch?(.*)$";
...
createWebViewWithConfiguration:(WKWebViewConfiguration *)configuration
   forNavigationAction:(WKNavigationAction *)navigationAction
    ...
    if ([self.delegate respondsToSelector:@selector(playerView:shouldInteractWithUrl:)]) {
        [self.delegate playerView:self shouldInteractWithUrl:navigationAction.request.URL];
    }
    return nil;
}
...
- (BOOL)handleHttpNavigationToUrl:(NSURL *)url {
    ...
    NSRegularExpression *yt2Regex =
    [NSRegularExpression regularExpressionWithPattern:kYTPlayerUrlRegexPattern
                                              options:NSRegularExpressionCaseInsensitive
                                                error:&error];
    NSTextCheckingResult *yt2Match =
    [yt2Regex firstMatchInString:url.absoluteString
                         options:0
                           range:NSMakeRange(0, [url.absoluteString length])];
    ...
    if (ytMatch || yt2Match || adMatch || oauthMatch || staticProxyMatch || syndicationMatch) {
        return YES;
    } else {
        if ([self.delegate respondsToSelector:@selector(playerView:shouldInteractWithUrl:)]) {
            [self.delegate playerView:self shouldInteractWithUrl:url];
        }
        return NO;
    }
}
